...
source of the loss.  Thus we believe that the main source of the
negative effect digital normalization had on the Trinity assembly was
on the algorithms used for determining likely transcripts.
...
is {\em single-pass}: the algorithm does not need to look at any read
more than once.  Moreover, because reads are accumulated sub-linearly
with over any deep data set, likely errors also do not accumulate
linearly, and the overall memory usage is consequently limited.
Combined, these features let us adopt a streaming approach to the data
reduction of extremely large data sets, with all of the advantages of
treating sequence as a data stream (@@).
