...
% @@comparison with dnadiff/GAGE
%

% Faster sequencers, larger datasets, new challenges
% Mason?CE, Elemento?O
% Genome Biology?2012, 13:314 (27 March 2012)       
% Abstract                http://genomebiology.com/2012/13/3/314/abstract
% Full text               http://genomebiology.com/2012/13/3/314
% PDF
% http://genomebiology.com/content/pdf/gb-2012-13-3-314.pdf

%% can now apply error correction to non-genomic@@
%% impact on assembly
%% remove redundant sequences
%% metagenomics etc
%% scale with sequence content etc.
%% will trimble's arg
%% Preprocessing RNA scaffolding, tip hat gene models, error corr
%% GAGE argues that quality of reads => quality of assembly.  we get good qual!
%% nature review on de novo assembly
...
\title{A reference-free algorithm for computational normalization of shotgun sequencing data}
...
sampling variation in short-read data sets and the high sequencing
error rate of modern sequencers present many new computational
...
assemblers.  These algorithms continue to be challenged by the
...
Velvet, SOAPdenovo, ALLPATHS, and SGA
\cite{pubmed19251739,pubmed18349386,pubmed20511140,pubmed21187386,pubmed22156294}.
...
variation in sequence content from molecular amplification techniques @@.
...
reference data is needed.  Digital normalization is inspired by
experimental normalization techniques developed for cDNA library
preparation, in which hybridization kinetics are exploited to reduce
the copy number of abundant transcripts prior to
sequencing\cite{pubmed8889548,pubmed7937745}.  {\em Digital}
...
sampling variation while removing reads, and also removing the many
errors contained {\em within} those reads.  This data and error
reduction results in decreased computational requirements for {\em de
  novo} assembly.  Unlike experimental normalization where abundance
information is experimentally removed, in digital normalization this
information can be recovered from the unnormalized reads.
...
assembled reference genomes (see @@ for details).  The wide variation
in coverage for the two MDA data sets is due to the amplification
procedure @@.  After normalizing to a k-mer coverage of 20, the high
coverage loci are systematically shifted to an average of XX@@, while
low-coverage loci remain at their previous coverage.  This smooths
...
addition, in most cases the process of sequencing fails to recover
more true k-mers (parentheses, middle column) than digital
normalization discards (parentheses, fourth column).
...
a 1\% per-base substitution error rate, each 100-bp read will have 1
substitution error on average. Each of these substitution errors will
introduce up to $k$ erroneous k-mers.  Thus, for each read we discard
as redundant, we also eliminate an average of $k$ erroneous k-mers.
...
reads) \cite{@@,@@}.  By calculating per-locus coverage in a way that
is partially refractory to errors, digital normalization converts
...
\cite{muthukrishnan2005data}.  This enables increasingly efficient
downstream analyses, including e.g.  quantification of mRNAseq and
homology search of large metagenomic data sets (@@pachter, @@MG-RAST)
...
determining community and transcript complexity as well as calculating
the required sequencing depth for sensitive analyses.

More pragmatically, digital normalization scales existing assembly
techniques dramatically.  By reducing overall sampling variation,
digital normalization could also lead to improved contig assembly,
which may be happening in the case of MDA samples (see @@).  The data
size reduction also may enable the application of
overlap-layout-consensus assembly algorithms to short-read data.
Overall, the reduction in data set size, memory requirements, and time
complexity for contig assembly afforded by digital normalization could
lead to the application of more complex heuristics to the assembly
problem.
...
entirely preserved by digital normalization (see tables @@).
\cite{pubmed20211242}.
...
and assembling across repeats are all difficult tasks, and continue to
...
by SGA and Bambus2 \cite{pubmed20529929,pubmed21926123}.
...
recovering complete or nearly complete contig assemblies.
Normalization is a substantially different approach than uniform
downsampling: by doing downsampling in a locus-specific manner, we
retain low coverage data.  Previously decsribed approaches to reducing
sampling variation are {\em ad hoc} and may not be generally
applicable @@.

Digital normalization is a general strategy for scaling sequence
assembly and provides a basis for streaming approaches in sequence
analysis.  The reference-free estimator of per-read coverage offered
by the median k-mer count may also enable additional analyses of
coverage and complexity in shotgun data sets.
...
normalization can lead to significant overall time decreases in
assembly as well.
