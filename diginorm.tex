...
% @@spellcheck!
% fix E. coli, de novo.
% VERIFY coverage figure; 0 coverage, average coverage.
% Check distance-to-end-of-read-for-starting-position => rev comp
% todo:
...
% @why does coverage look like < 200x??
...
trimming after normalizing to a low coverage of $\tilde 5$ results in
significantly more lost k-mers, because there are regions in the
genome that by chance have yielded 5 reads with errors in them.  With
the three-pass approach, the second round of digital normalization can
retain error-free reads for this region.  And, because the first pass
of digital normalization discards substantial amounts of data as well
as many erroneous k-mers, these later passes are always less time and
...
MDA-amplified DNA from a single cell of an uncultured {\em
  Deltaprotobacterium}.  For all three samples, the number of reads remaining
after digital normalization was reduced by at least 30-fold, while the
memory and time requirements were reduced 10-100x.
...
We then assembled both the original and normalized sequence reads with
Oases and Trinity, two de novo transcriptome assemblers.  (Note that due
to differing execution parameters, the Oases runtimes cannot be compared
...
and unnormalized Trinity and Oases assemblies.  Curiously, digital
normalization resulted in {\em fewer} contigs in the normalized Oases
assembly, and {\em more} contigs in the normalized Trinity assembly,
indicating that digital normalization affected the assemblers
differently.  Using a local-alignment-based overlap analysis (see Methods)
we found that the normalized Oases assembly had an 98.5\% overlap with
the unnormalized Oases assembly, while the normalized Trinity assembly
had a 97\% overlap with the unnormalized Trinity assembly.  This suggests
that the differences in the assemblies are caused by elimination of splice
variants in Oases, and fragmentation of long transcripts in Trinity.

@ read alignment?

@ end test?

@ how verify hypotheses?
...
single-cell amplified genomes @@. However, digital normalization is applied computionally --
...
is less sensitive to errors.  Our current approach also ignores
...
sequences in repeat-rich genomes.  Here, we have only
...
possible and it should be possible to scale digital normalization
